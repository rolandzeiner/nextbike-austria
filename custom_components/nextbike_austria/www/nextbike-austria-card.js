...
`;let Ce=class extends he{constructor(){super(...arguments),this._config={type:"nextbike-austria-card",entities:[]},this._toggleStation=e=>{const t=[...this._config.entities||[]],i=t.findIndex(t=>t.entity===e),s=i>=0?t.filter((e,t)=>t!==i):[...t,{entity:e}];this._config={...this._config,entities:s},this._fire()},this._setBool=(e,t)=>{this._config={...this._config,[e]:t},this._fire()},this._setLayout=e=>{"stacked"!==e&&"tabs"!==e||(this._config={...this._config,layout:e},this._fire())}}static{this.styles=Ee}setConfig(e){this._config=we(e)}_et(e){return function(e,t){const i=me(e);return ve(ye[i],["editor",t])??ve(ye.en,["editor",t])??t}(this.hass,e)}_fire(){this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:{...this._config}},bubbles:!0,composed:!0}))}render(){if(!this.hass)return Z;const e=$e(this.hass),t=this._config.entities||[],i=new Set(t.map(e=>e.entity)),s="tabs"===this._config.layout?"tabs":"stacked",n=!1!==this._config.show_rack;return q`
    ...
    `}_renderStationChip(e,t){const i=this.hass?.states[e]?.attributes,s=Se(i?.friendly_name||e),n=t.has(e);return q`
      <button
        type="button"
    ...
    `}_renderEmpty(){const e=$e(this.hass).length?"no_entities_picked":"no_entities_available";return q`<div class="empty-state">${this._t(e)}</div>`}_renderTabs(e){return q`
      <div class="tabs" role="tablist">
        ${e.map((t,i)=>{const s=Se((this.hass?.states[t.entity]?.attributes||{}).friendly_name||t.entity),n=i===this._activeTab;return q`
            <button
              type="button"
              role="tab"
              class="tab ${n?"active":""}"
    ...
    `}_renderPills(e,t,i){const s=[];if(this._config.show_ebikes&&"number"==typeof e&&Number.isFinite(e)&&e>0&&s.push(q`
      ...
      `)}return s}_renderRack(e){const{bikes:t,ebikes:i,capacity:n,accent:r,batteryPct:o,batterySamples:a,batteryList:l,vehicleTypesAvailable:c,vehicleTypeNames:d,reservedCount:h,reservedTypes:p,disabledCount:u,disabledTypes:b}=e,g=n,_=Math.min(t,n),f=Math.min(Number.isFinite(h)?h:0,Math.max(0,g-_)),y=Math.min(Number.isFinite(u)?u:0,Math.max(0,g-_-f)),m="number"==typeof i&&Number.isFinite(i)&&i>0,v=m?Math.min(_,i):0,x=!!this._config.show_battery&&"number"==typeof o&&a>0,$=x&&Array.isArray(l)?l:[],k=function(e,t){if(!Array.isArray(e))return null;for(const i of e){const e=String(i?.vehicle_type_id??"");if(s.has(e)&&t?.[e])return t[e]}return null}(c,d),w=function(e,t){const i=[];if(!Array.isArray(e))return i;for(const n of e){const e=String(n?.vehicle_type_id??""),r="number"==typeof n?.count&&Number.isFinite(n.count)?n.count:0;if(s.has(e)||r<=0)continue;const o=t?.[e]||"";for(let e=0;e<r;e++)i.push(o)}return i}(c,d);let A=0;const S=[];for(let e=0;e<_;e++){if(e<v){const t=$[e]||null,i=t?.type||k||this._t("legend_ebike");if(t&&x&&"number"==typeof t.pct){const e=t.pct,s=Ae(e),n=`${i} · ${Math.round(e)}%`;S.push(q`
            <div
          ...
          `)}else{const e=x?`${i} · ${this._t("battery_unknown")}`:i;S.push(q`
            <div
              ...
              title=${e}
            ></div>
          `)}}else{const e=w[A++]||this._t("legend_bike");S.push(q`
          <div
            class="slot filled"
            role="img"
            aria-label=${e}
      ...
      `)}for(let e=_+f+y;e<g;e++){const e=this._t("legend_empty");S.push(q`
        <div
          ...
          title=${e}
        ></div>
      `)}const T=t>n,z=_+f+y<g,P=f>0,M=y>0,R=this._t("rack_summary").replace("{available}",String(_)).replace("{capacity}",String(n));return q`
      <div class="rack" role="group" aria-label=${R}>
        ${S}
        ${T?q`<span class="rack-note" aria-label=${"+"+(t-n)}>+${t-n}</span>`:Z}
      </div>
      ${this._config.show_legend?this._renderLegend({accent:r,hasEbikes:m,hasOverflow:T,hasEmptyVisible:z,battery:x&&"number"==typeof o?{pct:o,color:Ae(o)}:null,hasReservedVisible:P,hasDisabledVisible:M}):Z}
    `}_renderLegend(e){const{accent:t,hasEbikes:i,hasOverflow:s,hasEmptyVisible:n,battery:r,hasReservedVisible:o,hasDisabledVisible:a}=e,l=[q`
