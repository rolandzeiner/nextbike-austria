...
const TRANSLATIONS = {
  de: {
    ...
    dock: "Platz",
    ebikes: "E-Bikes",
    docks_unknown: "Plätze unbekannt",
    capacity: "Kapazität",
...
class NextbikeAustriaCard extends HTMLElement {
  ...
  _renderStation(stopCfg) {
    ...
    if (this._config.show_docks) {
      ...
      } else if (a.is_virtual_station !== true) {
        // Capacity unpublished (e.g. Niederösterreich) — not a geofence,
        // just missing dock metadata. Show the status honestly.
        pills.push(
          `<span class="pill muted" title="${_esc(this._t("docks_unknown"))}">
             <ha-icon icon="mdi:help-circle-outline"></ha-icon>${_esc(this._t("docks_unknown"))}
           </span>`,
        );
      }
    }
    ...
    if (this._config.show_rack && capacity !== null && capacity > 0) {
      rackHtml = this._renderRack(bikes, ebikes, capacity, accent);
    }
  ...
  _renderRack(bikes, ebikes, capacity, accent) {
    ...
    for (let i = 0; i < bikesVis; i++) {
      ...
      slots.push(
        `<div class="slot filled${isEbike ? " ebike" : ""}" style="background:${
          isEbike
            ? `linear-gradient(135deg, ${accent} 0%, ${accent} 55%, #ffd740 55%, #ffd740 100%)`
            : accent
        };"></div>`,
      );
    }
    for (let i = bikesVis; i < totalSlots; i++) {
    ...
    const hasEmptyVisible = bikesVis < totalSlots;
    const legendHtml = this._config.show_legend
      ? this._renderLegend(accent, hasEbikes, hasOverflow, hasEmptyVisible)
      : "";
    return `${rackHtml}${legendHtml}`;
  }

  _renderLegend(accent, hasEbikes, hasOverflow, hasEmptyVisible) {
    ...
    if (hasEbikes) {
      items.push(
        `<span class="legend-item">
           <span class="legend-swatch" style="background:linear-gradient(135deg, ${accent} 0%, ${accent} 55%, #ffd740 55%, #ffd740 100%);"></span>
