...
// Suppress noisy node_modules `this` warnings (e.g. from CommonJS
// internals after Rollup converts them). Real warnings still surface.
...
export default {
  ...
  plugins: [
    ...
    typescript(),
    json(),
    !dev && terser({ format: { comments: /Nextbike Austria Card/ } }),
  ].filter(Boolean),
  onwarn,
};
